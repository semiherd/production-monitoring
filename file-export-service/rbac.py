...
def _get_jwks():
    ...
    url = current_app.config.get("AUTH_JWKS_URL")
    ...
    def decorator(f):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if not jwk: return jsonify({"error":"unknown kid"}), 401
                pub_pem = _public_key_from_jwk(jwk)
                claims = jwt.decode(token, pub_pem, algorithms=["RS256"], audience=current_app.config.get("JWT_AUDIENCE","smartline"), issuer=current_app.config.get("JWT_ISSUER","https://auth.service.local"))
